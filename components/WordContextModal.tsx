...
export default function WordContextModal({
  ...
  useEffect(() => {
    setLoading(true)
    setData(null)
    fetch(`/api/words/${word.id}/context?bookId=${bookId}`)
      .then((r) => r.json())
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-2xl max-h-[85vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-start justify-between px-5 py-3 border-b border-slate-100 shrink-0">
          ...
          <button
            onClick={onClose}
            className="mt-1 w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-100 active:scale-90 transition-all"
          >
        ...
        <div className="overflow-y-auto px-5 py-4 space-y-5">
          ...
          {/* Book sentences */}
          {data && data.bookSentences.length > 0 && (
            <section>
              <h3 className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-2">
                In this book
              </h3>
              <div className="space-y-2">
                {data.bookSentences.map((s, i) => (
                  <p
                    key={i}
                    className="text-sm text-slate-700 bg-slate-50 rounded-xl px-3 py-2.5 leading-relaxed"
                  >
            ...
            <section>
              ...
              {data.ai.explanation ? (
                <>
                  ...
                  {data.ai.examples.length > 0 && (
                    <div className="space-y-2">
                      {data.ai.examples.map((ex, i) => (
                ...
                <p className="text-sm text-slate-400 italic">
                  AI explanation unavailable — add ANTHROPIC_API_KEY to enable.
                </p>
              )}
            </section>
          )}
